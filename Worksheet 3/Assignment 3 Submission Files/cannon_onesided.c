...
int main (int argc, char **argv) {
	...
	double compute_time = 0, mpi_time = 0, total_time = 0, start_mpi, start_compute, start_total;
	int C_index, A_row, A_column, B_column;

	// Allocate memory for shared region. These are allocated locally for each process
	...
	// Declare windows for one-sided communication
	MPI_Win win_A;
	MPI_Win win_B;

	// Create windows for one-sided communication. Shared blocks are grouped by windows and made remotely accessible. 
	// By specifying the rank and window corresponding memory blocks will be accessible
	...
	for(cannon_block_cycle = 0; cannon_block_cycle < sqrt_size; cannon_block_cycle++){
		// Start accessing windows
		MPI_Win_fence(0, win_A);	
		MPI_Win_fence(0, win_B);
		// Start measuring mpi time
		start_mpi = MPI_Wtime();
		// Start the sending of local blocks to shared blocks
		MPI_Put(A_local_block, A_local_block_size, MPI_DOUBLE, (coordinates[1] + sqrt_size - 1) % sqrt_size, 0, A_local_block_size, MPI_DOUBLE, win_A);
		MPI_Put(B_local_block, B_local_block_size, MPI_DOUBLE, (coordinates[0] + sqrt_size - 1) % sqrt_size, 0, B_local_block_size, MPI_DOUBLE, win_B);	
		// Accumulate mpi time
		...
		compute_time += MPI_Wtime() - start_compute;
		// Start measuring mpi time
		start_mpi = MPI_Wtime();
		// Synchronisation for sending to shared blocks 
		MPI_Win_fence(0, win_A);
		MPI_Win_fence(0, win_B);
		// End measuring mpi time
		mpi_time += MPI_Wtime() - start_mpi;
		// Receive data from shared blocks to local blocks
		*A_local_block = *A_shared_block;
		*B_local_block = *B_shared_block;
	}
	// End measuring total time
	total_time = MPI_Wtime() - start_total;
	...
	free(B_local_block);
	free(C_local_block);
	// Free memory used for shared region
	free(A_shared_block);
	free(B_shared_block);
	// Free Windows
	 MPI_Win_free(&win_A);
